...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--task-name", type=str, required=True, help="Task name of dm-control-suite environment"
    )
    parser.add_argument("--n-train", type=int, default=500, help="Number of train episodes")
    parser.add_argument(
        "--n-test", type=int, default=1, help="Number of test episodes per evaluation"
    )
    parser.add_argument(
        "--test-every", type=int, default=10, help="Number of train episodes between evaluations"
    )
...
def run(args: argparse.Namespace) -> None:
    out_dir = Path(args.out_dir) / args.env / f"{time.time()}"
    out_dir.mkdir(parents=True, exist_ok=True)
    write_out_args(args=args, out_dir=out_dir)

    set_seeds()
    env = DMCWrapper(suite.load(domain_name=args.domain_name, task_name=args.task_name))
